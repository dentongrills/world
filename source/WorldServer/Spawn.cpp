...
void Spawn::InitializePosPacketData(Player* player, PacketStruct* packet){
	...
	if (IsPlayer()) {
		...
		packet->setDataByName("pos_unknown", static_cast<sint16>(GetSpeedY() * 32), 1);
		packet->setDataByName("pos_unknown", static_cast<sint16>(GetSpeedZ() * 32), 2);
	...
	if (speed_ratio > 0) {
		...
		if (IsPlayer()) {
			...
			packet->setDataByName("pos_unknown6", GetSideSpeed() * speed_multiplier, 3);
		} else {
